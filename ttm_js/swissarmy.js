...
if(this.descriptions)
    document.write(brs + '<div align="center"><font size="2" face="Arial, Helvetica, sans-serif">' + this.dp + '<span id="imgdsc' + this.issid + '">&nbsp;<\/span>&nbsp;&nbsp;ID: <span id="file" runat="server"><\/span><\/font><input type="hidden" name="idpic" id="idpic" value="1"\/></div>\n');
...
document.write('<div align="center"><font size="2" face="Arial, Helvetica, sans-serif">Fotografia : <b><span id="thecnt' + this.issid + '"><\/span><\/b>&nbsp;de&nbsp;&nbsp;<b>' + this.imgs.length.toString() + '<\/b><\/font><\/div>');
}

inter_slide.prototype.controls=function(){
var brs=this.addbr? '<br>' : ' &nbsp;';
if(!this.ics&&!this.no_c)
document.write('<div align="center"><input id="prev'+this.issid+'" disabled type="button" value=" << " onclick="iss['+this.issid+'].changeimg(false, \'nav\');fill(thecnt'+this.issid+'.innerHTML);">\n'+
(this.no_auto? '&nbsp;&nbsp;&nbsp;' : ' <input id="gostp'+this.issid+'" type="button" value=" Stop " onclick="iss['+this.issid+'].gostop(this);">\n')+
' <input id="next'+this.issid+'" disabled type="button" value=" >> " onclick="iss['+this.issid+'].changeimg(true, \'nav\');fill(thecnt'+this.issid+'.innerHTML);">'+(this.jumpto? brs+'Jump to: <input disabled type="text" size="'+this.imgs.length.toString().length+'" id="goto'+this.issid+'" onkeydown="iss['+this.issid+'].jumper(event);"><input id="go'+this.issid+'" disabled type="button" onclick="iss['+this.issid+'].jumper();" value="GO">' : '')+'<\/div>')
...
slideHTML+='<a href="'+this.imgs[picidx][2]+'"'+(this.imgs[picidx][3]? ' id="linkx" target="'+this.imgs[picidx][3]+'"' : this.target? ' target="'+this.target+'"' : '')+' onclick="'+(this.onclick? this.onclick : 'window.open(this.href, (this.target? this.target : \'_self\')'+specs+');return false;')+'">'
}
slideHTML+='<img id="theimg'+picidx+'_'+this.issid+'" width="'+(this.width-10)+'" onError=this.src="images/noDisponible.png" src="'+(this.loadimgidx[picidx]&&typeof this.loadimgidx[picidx].complete=='boolean'&&this.loadimgidx[picidx].complete? this.loadimgidx[picidx].src : this.imgs[picidx][0])+'" alt="'+(this.ualt? this.imgs[picidx][1] : 'Slide Show Image')+'" title="'+(this.utit? this.imgs[picidx][1] : '')+'" '+(this.imbcolor&&!this.imgs[picidx].noborder? 'style="border:'+this.imgborder+'px '+(this.imbstyle? this.imbstyle : 'solid')+' '+this.imbcolor+';"' : 'border="'+(this.imgs[picidx].noborder? '0' : this.imgborder)+'"')+(!this.width||!this.height? ' onload="iss['+this.issid+'].imgload(this);"' : '')+'>'
if (this.imgs[picidx][2]) //if associated link exists for img
slideHTML+='<\/a>'
...
if(this.descriptions)
    document.getElementById("file").innerHTML = this.imgs[this.keeptrack()][5];
    document.getElementById("HddFiles").value = this.imgs[this.keeptrack()][5];
