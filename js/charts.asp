...
function grEspesor2()
	...
	Response.Write "<dataset seriesName='PROYECCION " &  fechaT & " Tasa " & round(tasa,3) & "mm/día   Media " & round(media,3) & " Min " & min & "'  showValues='0' anchorRadius='2' color='FFCC00'>" 
...
function grProyec()
	...
	Response.Write "<trendLines>"
    Response.Write "<line startValue='200900' color='000000' displayvalue='Año Actual 2009' valueOnRight='1'/> "
	Response.Write "</trendLines>"
...
query = query  & " ORDER BY NUMERO "
	...
	ORs.MoveFirst
	Response.Write "<dataset seriesName='Mantención Programada'  showValues='0' color='FF0000'>" 
	While Not ORs.EOF 
	...
	ORs.MoveFirst
	Response.Write "<dataset seriesName='Fallas x Mantención'  showValues='0' color='94B0BE'>" 
	While Not ORs.EOF 
	...
	ORs.MoveFirst
	Response.Write "<dataset seriesName='Fallas x Operación'  showValues='0' color='00FF33'>" 
	While Not ORs.EOF 
...
query = query & "		ROUND(SUM(CASE WHEN TIPO IN ('OP') THEN MINUTOS END)/60,2) AS OP, COUNT(*) AS NF "
query = query & " FROM PARADAS AS P  "
query = query & " WHERE  P.SECCION in ('APILA')" 
query = query & "	  	    AND  FAENA = " & strNvl(FAENA) 
'query = query & "	  	    AND  MONTH(FECHA_INI)=" & MES & " AND YEAR(FECHA_INI)=" & ANHO 
...
query = query & " ORDER BY TT DESC "
	...
	Response.Write "<REGISTROS>"
	While Not ORs.EOF 
		Response.Write "<EQUIPO>" & CStr(ORs.Fields.Item(0).Value) &  "</EQUIPO>"
		Response.Write "<HORAS>" & REPLACE(ROUND(ORs.Fields.Item(4).Value/TTotal*100,2),",",".") &  "<HORAS>"
		ProcAcum = ProcAcum + (ORs.Fields.Item(4).Value/TTotal)
		Response.Write "<ACUMULADO>" & REPLACE(ROUND(ProcAcum*100,2),",",".")  &  "</ACUMULADO>"
		 ORs.MoveNext
	Wend
	Response.Write "</REGISTROS>"
...
query = query & " ORDER BY FM DESC "
	...
	ORs.MoveFirst
	Response.Write "<?xml version='1.0' encoding='ISO-8859-15'?>"
	Response.Write "<chart palette='2' caption='Grafica de Dispersión' shownames='1'  numDivLines='10' yAxisName='MTTR'  rotateNames='1'  anchorRadius='5' anchorBorderThickness='2' imageSave='1' imageSaveURL='FCExporter.php' showBorder='1' borderColor='000000' borderThickness='1' bgColor='FFFFFF'>"
...
query = query & " ORDER BY FM DESC "
	...
	ORs.MoveFirst
	Response.Write "<?xml version='1.0' encoding='ISO-8859-15'?>"
	Response.Write "<chart palette='2' caption='Grafica de Jack Knife' shownames='1'  numDivLines='10' XAxisName='Nº FALLAS' yAxisName='MTTR' bgColor='FFFFFF' showBorder='1' anchorRadius='5' anchorBgAlpha='50'  imageSave='1' imageSaveURL='FCExporter.php'>"
	Response.Write "<categories verticalLineThickness='1'>"
	While Not ORs.EOF 
		 MTBF = ROUND((ORs.Fields.Item(3).Value - ORs.Fields.Item(4).Value)/ isCero(ORs.Fields.Item(10).Value),2)
