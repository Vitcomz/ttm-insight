...
function getLayout()
	...
	query = query & " DBO.EquipoPrincipalFaena(B.EQUIPO, B.UBICACION) = EU.EQUIPO AND"
	query = query & " EU.USUARIO=" & strNvl(USUARIO)
	query = query & " LEFT OUTER JOIN EQUIPOS AS E ON B.EQUIPO = E.CODIGO AND E.UBICACION = B.UBICACION  "
...
function GetBitacora()
	...
	query = query & " B.CAUSA, B.RECOMENDACION, B.COLOR, B.IND_SOLUCION, B.EJECUTOR, B.ULT_INSPECCION, B.OT, B.ULT_MOD_OBS"
...
Set objMessage.Configuration = objConfig
	...
	'BODY = BODY & "OBSERVACIÓN:<strong> " & OBSERVACION & "</strong><BR><BR><BR>"
...
function InfBitPolines()
	...
	query = query & " and B.UBICACION=" & strNvl(FAENA) & " and case when  CHARINDEX('Derecho', E.DESCRIPCION) > 0 then 'DER' when  CHARINDEX('Centro', E.DESCRIPCION) > 0 then 'CEN'  ELSE 'IZQ' END = 'DER'  "
	...
	ELSE
		Response.ContentType = "text/xml"
		Response.Write "<?xml version='1.0' encoding='ISO-8859-15' ?>" 
		'Response.Write "<?xml-stylesheet type='text/xsl' href='../Informes/infPolines.xsl'?>" & VbCrLf
	END IF
