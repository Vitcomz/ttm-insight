...
function ListMenuxRol()
	...
	if EMPRESA <> "ANDINA" then
		query = query & "    AND NOMBRE <> 'Biblioteca'"
	end if
	...
	While Not ORs.EOF 
	  Response.Write "<ROW>"
		  Response.Write "<CODIGO>" & trim(ORs.Fields.Item(0).Value) & "</CODIGO>" & VbCrLf
  		  Response.Write "<COD_MENU_PADRE>" & trim(ORs.Fields.Item(1).Value) & "</COD_MENU_PADRE>" & VbCrLf
  		  Response.Write "<NOMBRE>" & trim(ORs.Fields.Item(2).Value) & "</NOMBRE>" & VbCrLf
		  Response.Write "<LINK>" & trim(ORs.Fields.Item(3).Value) & "</LINK>"&  VbCrLf
		  Response.Write "<TARGET>" & trim(ORs.Fields.Item(4).Value) & "</TARGET>" & VbCrLf
  		  Response.Write "<SISTEMA>" & trim(ORs.Fields.Item(5).Value) & "</SISTEMA>" & VbCrLf
		  Response.Write "<COD_ROL>" & trim(ORs.Fields.Item(6).Value) & "</COD_ROL>" & VbCrLf
  		  Response.Write "<ORDEN>" & trim(ORs.Fields.Item(7).Value) & "</ORDEN>" & VbCrLf
	  Response.Write "</ROW>"
	  ORs.MoveNext 
	Wend
	Response.Write "</REGISTROS>"
...
function ListSubMenu()
	...
	If strNvl(COD_MENU_PADRE)<>"NULL" Then	
		If strNvl(COD_MENU_PADRE)<>"'TODOS'" Then
	...
	query = query & "  ORDER BY COD_MENU_PADRE, ORDEN"
	
	Set ORs =oDB.EjecutaSql(query)

	Response.Write "<?xml version='1.0'  encoding='ISO-8859-1'?>" & VbCrLf & VbCrLf
	Response.Write "<REGISTROS>"
	 rem Response.Write "<CODIGO>" & query & "</CODIGO>"
	While Not ORs.EOF 
	  Response.Write "<ROW>" 
		  Response.Write "<CODIGO>" & ORs.Fields.Item(0).Value & "</CODIGO>"
		  Response.Write "<COD_MENU_PADRE>" & ORs.Fields.Item(1).Value & "</COD_MENU_PADRE>"
...
function DelMenu()
	...
	query = "DELETE FROM MENU WHERE CODIGO=" & strNvl(CODIGO) 
