...
function ListMenuxRol()
	...
	While Not ORs.EOF 
	  Response.Write "<ROW>"
		  Response.Write "<CODIGO>" & trim(ORs.Fields.Item(0).Value) & "</CODIGO>"
  		  Response.Write "<COD_MENU_PADRE>" & trim(ORs.Fields.Item(1).Value) & "</COD_MENU_PADRE>"
  		  Response.Write "<NOMBRE>" & trim(ORs.Fields.Item(2).Value) & "</NOMBRE>"
		  Response.Write "<LINK>" & trim(ORs.Fields.Item(3).Value) & "</LINK>"
		  Response.Write "<TARGET>" & trim(ORs.Fields.Item(4).Value) & "</TARGET>"
  		  Response.Write "<SISTEMA>" & trim(ORs.Fields.Item(5).Value) & "</SISTEMA>"
		  Response.Write "<COD_ROL>" & trim(ORs.Fields.Item(6).Value) & "</COD_ROL>"
  		  Response.Write "<ORDEN>" & trim(ORs.Fields.Item(7).Value) & "</ORDEN>"
	  Response.Write "</ROW>"
	  ORs.MoveNext 
	Wend
	Response.Write "</REGISTROS>"
