...
function ListArchivos()
	...
	While Not ORs.EOF 
	  ...
	  Response.Write "<FILE_ID>" & ORs.Fields.Item(0).Value & "</FILE_ID>"
	  ...
	  SELECT CASE  UCASE(ORs.Fields.Item(8).Value)
		...
		CASE  "DOC" 
	  	...
	  	CASE  "PPT" 
	 		 Response.Write "<FILEIMAGE>images\ppt.jpg</FILEIMAGE>"
	    CASE ELSE
	  		 Response.Write "<FILEIMAGE>\sci\Files\" & ORs.Fields.Item(0).Value & "." & ORs.Fields.Item(8).Value & "</FILEIMAGE>"
	  END SELECT
	  
	  Response.Write "<FILEID>\sci\Files\" & ORs.Fields.Item(0).Value & "." & ORs.Fields.Item(8).Value & "</FILEID>"
	  Response.Write "</ROW>"
	  ORs.MoveNext
	Wend
	Response.Write "</REGISTROS>"
