...
if (id_br == "FF" || id_br == "OP" || id_br == "SF"){
	xmlTmp = document.implementation.createDocument("", "", null)  
					...
					xmlTmp=xmlhttp.responseXML;
                    parser = new DOMParser();
            		xmlTmp = parser.parseFromString(xmlDoc, "text/xml"); 

	var x=xmlTmp.getElementsByTagName("ROW");
//	for (i=0;i<x.length;i++)
//	  { 
//	  alert(x[i].getElementsByTagName("CODIGO")[0].childNodes[0].nodeValue);
//	  alert(x[i].getElementsByTagName("NOMBRE")[0].childNodes[0].nodeValue);
//	  }
	//xmlTmp=loadXMLDoc(url);
}else if (id_br == "IE"){
			...
			//alert(tree.childNodes.length);
			   if (tree.childNodes.length > 0)
			   {
				for (var i = 0; i < tree.childNodes.length; i++){
					...
					var node = tree.childNodes.item(i);
					//str = getChildNodeValue(node,'CODIGO');
					oCMenu.makeMenu(getChildNodeValue(node,'CODIGO'), getChildNodeValue(node,'COD_MENU_PADRE'),DetectAcento(getChildNodeValue(node,'NOMBRE')),getChildNodeValue(node,'LINK'));
