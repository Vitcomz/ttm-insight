...
if f1 <> "" and f2 <> "" and faena <> "" then
	...
	query = "set dateformat dmy SELECT row_number() OVER (ORDER BY B.ID) AS NUMERO, B.ID, B.FECHA, B.RESPONSABLE, B.DESCRIPCION, B.IND_INCIDENTE, "
	query = query & " B.EQUIPO AS COMPONENTE, B.COLOR AS COLOR_CRITICIDAD, ISNULL(B.CRITICIDAD,''), ISNULL(B.CAUSA, ''), B.UBICACION AS FAENA, "
	query = query & " DATEDIFF ( dd , B.FECHA , SYSDATETIME() ) AS DIAS, dbo.EquipoPrincipalFaena(B.EQUIPO, B.UBICACION) AS EQUIPO_PRI, "
	query = query & " E.DESCRIPCION AS DESC_EQUIPO, ISNULL(B.RECOMENDACION, ''), B.REPORT AS OT, B.IND_SOLUCION, isnull(F.ARCHIVOS,' ') as ARCHIVOS, "
	query = query & " H.ULT_ACT AS ULT_ACT, EQ.TIPO_EQUIPO, FALLAS.DESCRIPCION "
	query = query & " FROM BITACORAS B  "
	query = query & " INNER JOIN EQUIPOS E ON  B.UBICACION = E.UBICACION AND dbo.EquipoPrincipalFaena(B.EQUIPO, B.UBICACION)=E.CODIGO " 
	query = query & " LEFT JOIN (SELECT IDENTIFICADOR, COUNT(0) AS ARCHIVOS from ARCHIVOS WHERE FAMILIA = 'BITACORAS' GROUP BY IDENTIFICADOR) AS F"
	query = query & " ON B.ID = F.IDENTIFICADOR  "
	query = query & " LEFT JOIN (SELECT ID, MAX(FECHA) AS ULT_ACT FROM HISTORIAL_BITACORA GROUP BY ID) AS H ON B.ID = H.ID "
	query = query & " LEFT JOIN (SELECT CODIGO, DESCRIPCION, TIPO_EQUIPO FROM EQUIPOS) AS EQ ON B.EQUIPO = EQ.CODIGO "
	query = query & " LEFT JOIN (SELECT     CHK.ID, CHK.DESCRIPCION, CHK_E.BITACORA, MAX(CHK_E.VALOR) as VALOR, CHK.TIPO_EQUIPO " 
	query = query & " FROM dbo.CHECKLIST AS CHK INNER JOIN dbo.CHECK_EQUIPOS AS CHK_E ON CHK.ID = CHK_E.CHECKLIST"
	query = query & " GROUP BY CHK.ID, CHK.DESCRIPCION, CHK_E.BITACORA, CHK.TIPO_EQUIPO) AS FALLAS ON EQ.TIPO_EQUIPO = FALLAS.TIPO_EQUIPO " 
	query = query & " AND B.ID = FALLAS.BITACORA WHERE B.UBICACION = '"&faena&"' AND B.FECHA BETWEEN '"&f1&"' AND '"&f2&"' ORDER BY H.ULT_ACT DESC" 
