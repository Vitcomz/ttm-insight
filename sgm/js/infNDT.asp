...
function newNDT()
	...
	query = "SELECT MAX(FOLIO) AS N FROM INFORMES "
	...
	query = "INSERT INTO INFORMES (FOLIO, FECHA,  RESPONSABLE, "
	query = query & " DESCRIPCION, ESTADO, TIPO, EQUIPO,  "
	query = query & " IND_SOLUCION, FAENA, "
...
function updateNDT()
	...
	query = "UPDATE INFORMES SET FECHA = " & strNvl(FECHA) & ","
...
function getNDT()
	...
	query = "SELECT I.FOLIO,  I.FECHA, I.RESPONSABLE, I.DESCRIPCION ,"
	query = query & "I.EQUIPO,  I.ESTADO, I.TIPO, I.IND_SOLUCION "
	query = query & " FROM INFORMES AS I "
	query = query & " INNER JOIN EQUIPOS AS E1 ON DBO.EQUIPOPRINCIPALFaena(I.EQUIPO,'" & FAENA & "') = E1.CODIGO "
	query = query & "  AND E1.UBICACION = I.FAENA"
	query = query & " INNER JOIN EQUIPOS AS E ON I.EQUIPO = E.CODIGO AND E.UBICACION = I.FAENA "
	query = query & " WHERE FOLIO =" & FOLIO 
	...
	If Not ORs.EOF Then
	  Response.Write "<ROW>" 
		  ...
		  Response.Write "<IND_SOLUCION>" & ORs.Fields.Item(7).Value & "</IND_SOLUCION>"
...
function listInf()
	...
	query = "SELECT I.FOLIO, I.TIPO, I.FECHA, I.RESPONSABLE, I.DESCRIPCION ,"
	query = query & " I.EQUIPO, I.ESTADO "
	query = query & " FROM INFORMES AS I "

	query = query & " WHERE I.FAENA=" & strNvl(FAENA)
	...
	While Not ORs.EOF 
	  ...
	  Response.Write "<ROW>" 
		  ...
		  Response.Write "<TIPO>" & ORs.Fields.Item(1).Value & "</TIPO>"
  		  ...
  		  Response.Write "<EQUIPO>" & ORs.Fields.Item(5).Value & "</EQUIPO>"
		  
		  SELECT CASE ORs.Fields.Item(6).Value
		  	CASE "1" 
			 Response.Write "<IMG_EST>mg/importancia_alta.gif</IMG_EST>"
			 Response.Write "<IMG_TEXT>Equipo Critico</IMG_TEXT>"
			CASE "2" 
			 Response.Write "<IMG_EST>mg/importancia_media.gif</IMG_EST>"
			 Response.Write "<IMG_TEXT>Importancia Alta</IMG_TEXT>"
			CASE "3" 
			 ...
			 CASE "4" 
			 Response.Write "<IMG_EST>mg/importancia_baja.gif</IMG_EST>"
			 Response.Write "<IMG_TEXT>Equipo en Buen estado.</IMG_TEXT>"
		  END SELECT 
		  
		   SELECT CASE ORs.Fields.Item(1).Value
		  	
			CASE "1"  
				Response.Write "<DESC_TIPO>RX</DESC_TIPO>"
				Response.Write "<IMG_TIPO>mg/rxIcon.jpg</IMG_TIPO>"
			
			CASE "2"  
				Response.Write "<DESC_TIPO>Scanner</DESC_TIPO>"
				Response.Write "<IMG_TIPO>mg/scannerIcon.jpg</IMG_TIPO>"
			
			CASE "3" 
				 Response.Write "<DESC_TIPO>Termografia</DESC_TIPO>"
			
			CASE "4"
				 Response.Write "<DESC_TIPO>Ultrasonido</DESC_TIPO>"
			
			CASE "5"
				 Response.Write "<DESC_TIPO>Informe de Fallas</DESC_TIPO>"
			
			CASE "6"
				Response.Write "<DESC_TIPO>Certificado de Empalme</DESC_TIPO>"
...
function listAvisosInd()
	...
	query = query & "  AND E1.UBICACION = B.UBICACION"
	query = query & " INNER JOIN EQUIPOS AS E ON B.EQUIPO = E.CODIGO AND E.UBICACION = B.UBICACION "
	query = query & " INNER JOIN SECCIONES AS A ON E1.SECTOR = A.CODIGO AND A.FAENA=" & strNvl(UBICACION)
	query = query & " WHERE B.UBICACION=" & strNvl(UBICACION)
	query = query & " 		AND  B.DESCRIPCION IS NOT NULL "
	...
	If  LEN(CRITICIDAD)>0 Then
		query = query & " AND B.ESTADO=" & strNvl(CRITICIDAD)
