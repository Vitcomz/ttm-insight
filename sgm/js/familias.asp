...
function NewFamilia()
	...
	query = "INSERT INTO FAMILIAS (CODIGO, DESCRIPCION, EMPRESA)"
	query = query & " VALUES ("
	query = query & strNvl(CODIGO) & "," 
	query = query & strNvl(DESCRIPCION) & ","
	query = query & "'SIMMATRANS')"
...
function ListFamilias()
	...
	query = query & " WHERE EMPRESA='SIMMATRANS' "
	
	IF IND_EXISTENCIA="YES" THEN
	query = query & " AND UPPER(IND_EXISTENCIA) = 'TRUE' "
	END IF
