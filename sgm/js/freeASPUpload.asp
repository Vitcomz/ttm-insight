...
Class FreeASPUpload
	...
	Public Sub Save(path)
		...
		For Each fileItem In UploadedFiles.Items
			...
			query = "SELECT ISNULL(MAX(ID),0) AS NUM FROM ARCHIVOS " 'WHERE EMPRESA=" & strNvl(EMPRESA)	
		...
		'Upload1.fileName_ = "hr"
    	   	...
    	   	query = "INSERT INTO ARCHIVOS (ID, NOMBRE, TIPO, SIZE, FECHA, RUTA, FAMILIA,"
		   	query = query & "  EXT, IDENTIFICADOR, EMPRESA) "
		  	query = query & " VALUES ("
			query = query & N & ","
			query = query & strNvl(fileItem.FileName) & ","
			query = query & strNvl(fileItem.ContentType) & ","
			query = query & strNvl(fileItem.Length) & ","
			query = query & "getdate(),"
		...
		'response.write query
			...
			If len(fileName_)>0 Then
				...
				response.write path
				streamFile.SaveToFile path  & N & "." & ext_ , 2
			Else
				streamFile.SaveToFile path  & N & "." & ext_ , 2
				'streamFile.SaveToFile path  & fileItem.FileName , 2
			End If
	...
	Private Sub Upload()
		...
		if Err.Number <> 0 then 
			...
			response.write "error is the incorrect setup of AspMaxRequestEntityAllowed in IIS MetaBase. <A HREF='http://www.freeaspupload.net/freeaspupload/requirements.asp'>requirements page of freeaspupload.net</A>.<p>"
		Exit Sub
		end if
		...
		Do Until nDataBoundPos = nLastSepPos
			...
			If nPosFile <> 0 And  nPosFile < nPosBound Then
				...
				if (Len(oUploadFile.FileName) > 0) then 'File field not left empty
					...
					If oUploadFile.Length > 0 Then UploadedFiles.Add LCase(sFieldName) & nCurPos, oUploadFile
				End If
			Else
				Dim nEndOfData
				nCurPos = FindToken(tNewLine, nCurPos) + 4 'skip empty line
