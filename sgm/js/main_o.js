...
function login(_plt){
	...
	//	window.open(url);
	 ...
	 $.get(url, function(theXML){
			...
			if (_value=="True"){
				crearSession(document.getElementById('usuario').value);
				//alert($(this).find("ROL").text());
				crearRol($(this).find("ROL").text());
				crearSessionArea(formulario.listFaenas.value);		
				
				//alert(getEmpresa());
				
				if(_plt)
					window.location.href="layout.html?FAENA=" + formulario.listFaenas.value;
				else
