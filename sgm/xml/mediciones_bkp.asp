...
function getMedicion()
	...
	query = query & " T.DESCRIPCION, M.OBSERVACION, M.DUREZA, M.DISTANCIA, E.UBICACION, MV.TIPO, count(0) as TOTAL, M.DIAS_OPERACION  " 
	query = query & " FROM MEDICIONES AS M INNER JOIN MEDICIONES_VALOR AS MV  ON M.FOLIO=MV.FOLIO, EQUIPOS AS E, TIPO_EQUIPOS AS T "
	...
	query = query & " GROUP BY M.FOLIO, M.FECHA , M.EQUIPO, E.DESCRIPCION, M.INSPECTOR, M.DIAS_OPERACION, M.FECHA_CAMBIO, M.DURACION, M.FECHA_ULTM_MANTENCION, E.ANCHO, E.ESPESOR, E.ESPESOR_R,  T.DESCRIPCION, M.OBSERVACION, M.DUREZA, M.DISTANCIA, E.UBICACION, MV.TIPO order by  E.UBICACION" 
	...
	While Not Rs.EOF 
		Response.Write "<ROW>"
		 ...
		 Response.Write "<TIPO>" & Rs.Fields.Item(16).Value & "</TIPO>"
		 Response.Write "<TOTAL>" & Rs.Fields.Item(17).Value & "</TOTAL>"
		 Response.Write "<FAENA>" & Rs.Fields.Item(15).Value & "</FAENA>"
		 Response.Write "<DIAS_OPERACION>" & Rs.Fields.Item(18).Value & "</DIAS_OPERACION>"
...
function newMedicion()
	...
	query = "SELECT FOLIO AS N FROM MEDICIONES WHERE FAENA = " & strNvl(FAENA) & " AND FECHA = " & strNvl(FECHA) & " AND EQUIPO = " & strNvl(EQUIPO) & " AND INSPECTOR = " & strNvl(INSPECTOR)
...
query =	" SELECT M.FOLIO, M.FECHA, M.EQUIPO, M.INSPECTOR, MV.TIPO, count(0) as TOTAL"
...
END IF
query = query & " GROUP BY M.FOLIO, M.FECHA, M.EQUIPO, M.INSPECTOR,  MV.TIPO"
	...
	While Not ORs.EOF 
	  Response.Write "<ROW>"  
		  ...
		  Response.Write "<TIPO>" & ORs.Fields.Item(4).Value & "</TIPO>"
		   ...
		   Response.Write "<FAENA>" & FAENA & "</FAENA>"
...
function listMedValor()
	...
	query = "SELECT ID, VALOR, RETORNO, FAENA, MEDICIONES_VALOR.folio, MEDICIONES_VALOR.tipo "
	query = query & " FROM MEDICIONES_VALOR INNER JOIN dbo.MEDICIONES ON dbo.MEDICIONES_VALOR.FOLIO = dbo.MEDICIONES.FOLIO "
	query = query & " WHERE FAENA in (" & strNvl(xFaena) & ")"
	'query = query & " AND TIPO= " & strNvl(TIPO)
	query = query & " ORDER BY faena, tipo, folio, id "
...
if(WIN = "OK") then
	 Response.Write ("<html><body><script>setTimeout(function(){ window.close(); }, 800);</script></body></html>")
