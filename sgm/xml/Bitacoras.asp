...
function ListBitacora()
	...
	query = query & " RECOMENDACION, B.CRITICIDAD, E.GCODE, E.X, E.Y, B.EJECUTOR, isnull(B.REPORT,' ') "
...
'				query = query & " AND DBO.SecEquipoPrincipal(B.EQUIPO) IN (" & SECCION & ")"
	...
	If IND_SOLUCION="FALSE" Then
		query = query & " AND (IND_SOLUCION IS NULL OR IND_SOLUCION= 'FALSE') "
	Else
	...
	query = query & " RECOMENDACION, B.CRITICIDAD, E.GCODE, X, Y, EJECUTOR, REPORT"
	...
	While Not ORs.EOF 
	  ...
	  Response.Write "<ROW>" 
		  ...
		  Response.Write "<AVISO>" & ORs.Fields.Item(21).Value & "</AVISO>" 
...
function UpdateBitacora()
	...
	query = "set dateformat DMY UPDATE BITACORAS SET FECHA = " & strNvl(FECHA) & ","
...
function LayoutFaenaPlanta()
	...
	ELSE
		query = query & " AND DBO.TipoEquipoPrincipal(B.EQUIPO) <> (17) " 
	END IF 
...
Set objMessage.Configuration = objConfig
	...
	BODY = BODY & "<title>Aviso de Inspección TTM Insight</title>" & VbCrLf
	BODY = BODY & "<style type='text/css'>body{margin-left:5px;margin-top:0px;font-family:Verdana, Tahoma, Arial;}</style>"
	BODY = BODY & "<body></br>"
	BODY = BODY & "Estimado Usuario: </br>"
	BODY = BODY & "Le informamos que se ha registrado una nueva inspección en TTMInsight.</br> "
	BODY = BODY & "<br><br>"
	BODY = BODY & "FAENA :<strong>" & FAENA & "</strong><BR>"
	BODY = BODY & "COMPONENTE: <strong> " & EQUIPO & "</strong><BR>"
