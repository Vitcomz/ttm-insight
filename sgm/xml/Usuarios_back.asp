...
function GetUsuario()
	...
	If Not ORs.EOF Then
	  Response.Write "<ROW>" 
		  ...
		  Response.Write "<RUT>" & ORs.Fields.Item(2).Value & "-" & ORs.Fields.Item(3).Value & "</RUT>"
...
function listFaenas()
	query = "select f.id, f.descripcion from faenas_usuario as fu, faenas as f "
	query = query & "where fu.id_faena=f.id and f.estado is not null and fu.usuario=" & strNvl(ID) & " order by f.descripcion"
