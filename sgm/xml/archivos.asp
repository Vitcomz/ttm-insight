...
function ListArchivos()
	...
	query = "SELECT ID, NOMBRE, EXT, RTRIM(CONVERT(CHAR,FECHA,103)) AS FECH, "
	query = query & " RUTA, FAMILIA, IDENTIFICADOR, SIZE, EXT "
	  ...
	  SELECT CASE  UCASE(ORs.Fields.Item(8).Value)
		...
		CASE  "DOCX" 
	  	...
	  	CASE  "PPTX" 
	 		 Response.Write "<FILEIMAGE>\sgm\images\ppt.jpg</FILEIMAGE>"			 
	    CASE ELSE
	  		 Response.Write "<FILEIMAGE>\sgm\files\imagen.aspx?image=/sgm/files/" & ORs.Fields.Item(0).Value & "." & ORs.Fields.Item(8).Value & "</FILEIMAGE>"
	  END SELECT
...
function SaveFile
  ...
  Dim fileClass, Upload, fileName, fileSize, ks, i, fileKey, resumen
	...
	query = "INSERT INTO ARCHIVOS (ID, NOMBRE, TIPO, SIZE, FECHA, RUTA, FAMILIA, EXT, IDENTIFICADOR, ORDEN, CLASIFICACION) "
	query = query & " VALUES ("
	query = query & N & ","
	...
	query = query & strNvl(CLASIFICACION) & ")"
