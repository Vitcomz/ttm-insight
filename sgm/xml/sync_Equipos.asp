...
function ListEquiposPadres()
	...
	query = "SELECT E.CODIGO, E.DESCRIPCION, E.LAYOUT, UPPER(EU.USUARIO), E.UBICACION "
	query = query & " FROM dbo.EQUIPOS AS E INNER JOIN dbo.EQUIPOS_USUARIOS AS EU ON E.CODIGO = EU.EQUIPO INNER JOIN dbo.USUARIOS AS US ON EU.USUARIO COLLATE SQL_Latin1_General_CP1_CI_AS = US.ID "
	
	If CODIGO="NULL" Then
		query = query & " WHERE E.COD_EQUIPO_PADRE IS NULL "
		query = query & " AND  E.CODIGO = EU.EQUIPO  AND  US.ESTADO = 'ACTIVO' "
	...
	if FAENA = "CERRO COLORADO" then
		query = query & " AND E.DESCRIPCION NOT like '%MOVIL%' AND E.TIPO_EQUIPO <> 17 AND E.TIPO_EQUIPO <> 4 "
	end if

	query = query & " AND E.UBICACION in (" & strNvl(xFaena) & ")"
	query = query & " AND E.ESTADO='ACT' "
	query = query & " GROUP BY E.CODIGO, E.DESCRIPCION, E.LAYOUT, E.SECUENCIA, EU.USUARIO, E.UBICACION  "
	query = query & " ORDER BY E.UBICACION, E.SECUENCIA "
	...
	While Not ORs.EOF 
	  Response.Write "<ROW>" 
		  ...
		  Response.Write "<USUARIO>" & ORs.Fields.Item(3).Value & "</USUARIO>"
...
function GetTipo()
	...
	query = query & "  E.CODIGO, E.UBICACION"
	...
	query = query & " AND E.UBICACION in (" & strNvl(xFaena) & ") and (e.codigo not like '%PCAR-P%' and e.codigo not like '%PCAR-E%' "
	query = query & " and e.codigo not like '%EST%' and e.codigo not like '%PRET-P%' and e.codigo not like '%PRET-E%' and e.codigo not like '%ZDI-P%' and e.codigo not like '%ZDI-E%') "
	...
	While Not ORs.EOF 
	  Response.Write "<ROWS>" 
		  ...
		  Response.Write "<COD>" & ORs.Fields.Item(3).Value & "</COD>"
		  Response.Write "<FAENA>" & ORs.Fields.Item(4).Value & "</FAENA>"
	  Response.Write "</ROWS>"
	  ORs.MoveNext 
	Wend
...
function GetTipoEST()
	...
	query = "SELECT E.TIPO_EQUIPO, E.DESCRIPCION, E.COD_EQUIPO_PADRE, "
	query = query & "  E.CODIGO, E.UBICACION"
	query = query & " FROM EQUIPOS AS E, TIPO_EQUIPOS AS TE"
	query = query & " WHERE E.TIPO_EQUIPO = TE.CODIGO "
	'query = query & " AND  E.CODIGO = " & strNvl(CODIGO)
	query = query & " AND E.UBICACION in (" & strNvl(xFaena) & ") and (E.codigo like '%EST%' or E.codigo like '%PRET-E%' or E.codigo like '%PCAR-E%' or E.codigo like '%ZDI-E%') "
...
function GetTipoPCAR()
	...
	query = query & " WHERE E.TIPO_EQUIPO = TE.CODIGO "
	'query = query & " AND  E.CODIGO = " & strNvl(CODIGO)
	query = query & " AND E.UBICACION in (" & strNvl(xFaena) & ") and (E.codigo like '%PCAR-P%') "
...
function GetTipoPRET()
	...
	query = query & " WHERE E.TIPO_EQUIPO = TE.CODIGO "
	'query = query & " AND  E.CODIGO = " & strNvl(CODIGO)
	query = query & " AND E.UBICACION in (" & strNvl(xFaena) & ") and (E.codigo like '%PRET-P%') "
...
function GetTipoZDI()
	...
	query = query & " WHERE E.TIPO_EQUIPO = TE.CODIGO "
	'query = query & " AND  E.CODIGO = " & strNvl(CODIGO)
	query = query & " AND E.UBICACION in (" & strNvl(xFaena) & ") and (E.codigo like '%ZDI-P%') "
...
function GetEquipo()
	...
	query = query & " E.ESPESOR, E.TON_SEMANAL, E.ESPESOR_R, E.LAYOUT "
	query = query & "FROM "
	query = query & "  (SELECT E.*, Equipos.DESCRIPCION AS DESC_PADRE "
	query = query & "  FROM EQUIPOS AS E LEFT JOIN Equipos ON E.COD_EQUIPO_PADRE = Equipos.CODIGO "
	query = query & "  WHERE "
'	query = query & " E.CODIGO=" & strNvl(CODIGO) & " AND "
	query = query & " E.UBICACION=" & strNvl(FAENA) & ") "
	query = query & " AS E "
	...
	While Not ORs.EOF 
	  Response.Write "<ROW>" 
		  ...
		  Response.Write "<LAYOUT>" & ORs.Fields.Item(39).Value & "</LAYOUT>"
		  Response.Write "<USUARIO>" & ORs.Fields.Item(39).Value & "</USUARIO>"
	  Response.Write "</ROW>"
	ORs.MoveNext
	Wend
	Response.Write "</REGISTROS>"
...
function ListEquiposPadresAndina()
	...
	query = "SELECT CODIGO,  DESCRIPCION"
	query = query & " FROM EQUIPOS WHERE ESTADO = 'ACT' AND COD_EQUIPO_PADRE IS NULL AND UBICACION=" & strNvl(FAENA)
	...
	End If
	query = query & " GROUP BY CODIGO, DESCRIPCION "
	'query = query & " ORDER BY ESTOCADA "
...
function ListEstocadaAndina()
	...
	query = "SELECT COD_ESTOCADA,  ESTOCADA"
	query = query & " FROM POLINES WHERE  FAENA=" & strNvl(FAENA)
	
	If CODIGO="NULL" Then
		'query = query & " WHERE  "
	Else
		query = query & " AND COD_ESTOCADA='" & strNvl(CODIGO) & "'"
	End If
	query = query & " GROUP BY COD_ESTOCADA, ESTOCADA "
	query = query & " ORDER BY ESTOCADA "
...
function ListMesaAndina()
	...
	query = "SELECT COD_MESA,  MESA"
	query = query & " FROM POLINES WHERE  FAENA=" & strNvl(FAENA)
	
	If CODIGO="NULL" Then
		'query = query & " WHERE  "
	Else
		query = query & " AND COD_MESA='" & strNvl(CODIGO) & "'"
	End If
	query = query & " GROUP BY COD_MESA, MESA "
	query = query & " ORDER BY MESA "
