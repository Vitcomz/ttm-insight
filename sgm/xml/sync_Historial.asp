...
function listHisBit()
	...
	query = "SELECT B.ID, B.INSPECTOR, convert(char(10),B.FECHA,105)  + ' ' +  convert(char(18),B.FECHA, 108), B.STATUS"
	query = query & " FROM HISTORIAL_BITACORA as B INNER JOIN BITACORAS  ON B.ID = dbo.BITACORAS.ID where ubicacion in (" & strNvl(xFaena) & ")"
...
function newHisBit()
	...
	query = query & " FECHA, STATUS, USERDATE) "
	query = query & " VALUES ("
	query = query & ID & ","
	query = query & strNvl(INSPECTOR) & ","
	query = query & strNvl(FECHA &" "& Time()) & ","
	query = query & strNvl(ESTATUS) & ", GETDATE())"
