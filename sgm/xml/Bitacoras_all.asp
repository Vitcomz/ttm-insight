...
function InfSemInspec()
	...
	if month(date()) = 1 then
		...
		prev_mes = month(date())-1
	end if
	FECHA_INI = "1/" & prev_mes & "/" & year(date())
	FECHA_FIN = fncLastDay(month(date()), year(date())) & "/" & month(date()) & "/" & year(date())
	query = "SELECT ID,"
	query = query & "rtrim(convert(char,B.fecha,103)) + ' ' + "
	query = query & " substring(convert(char,B.fecha,108),1,5) as FECHA"
	query = query & ", TURNO, B.RESPONSABLE,"
	...
	If FAENA<>"TOD" Then
		query = query & " AND  B.UBICACION=" & strNvl(FAENA) 
	End if
	query = query & " AND B.UBICACION=E.UBICACION"
	...
	ELSE
		Response.ContentType = "text/xml"
		Response.Write "<?xml version='1.0' encoding='ISO-8859-15' ?>" 
		Response.Write "<?xml-stylesheet type='text/xsl' href='../Informes/Inspecciones_all.xsl'?>" & VbCrLf
	END IF
