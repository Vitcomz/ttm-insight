...
function grEspesor2()
	...
	While Not ORs.Eof
		...
		IF FLAT=0 THEN
			...
			Response.Write "<chart palette='1' caption='MEDICION DE ESPESORES   " & EQUIPO & " - " & FAENA & "' shownames='1'  showvalues='1' overlapBars='1' showShadow='1' yAxisMaxValue='" & CUBIERTA & "' yAxisMinValue='" & FONDO & "'  legendPosition='BOTTOM'>" & VbCrLf
			...
			While Not ORsLecturas.Eof
		 		Response.Write "<category label='" & i & "' />" & VbCrLf
		...
		Response.Write "<dataset seriesName='MUESTREO " & ORs.Fields.Item(1).Value & "'  showValues='0' anchorRadius='2' lineThickness='2' color='" & vect(y) & "'>"  & VbCrLf
		fecha_muestreox = ORs.Fields.Item(1).Value
		While Not ORsLecturas.eof
		 		Response.Write "<set label='" & ORsLecturas.Fields.Item(0).Value _
					 & "' value='" & ORsLecturas.Fields.Item(1).Value & "' />" & VbCrLf
				'Response.Write(aMuestreo(i))
			i=i+1
			ORsLecturas.movenext
		Wend
		
		Response.Write "</dataset>" & VbCrLf
		ORs.MoveNext	
		y = y + 1
	...
	query ="UPDATE MEDICIONES SET TASA=" & round(tasa,3) & " ,"  
	query = query & " MEDIA= " & round(min,3) & " ," 
	query = query & " FECHA_ULTM_MANTENCION =ISNULL(FECHA_ULTM_MANTENCION, '" & FECHA_INST & "' ) "  
	...
	Else
		diasP  = (CUBIERTA-ESPESOR_SEG)
	End if
	
'	response.write "->"&CUBIERTA-ESPESOR_SEG&"->"&diasOP
'	response.write err.description
'	response.end
	
    fechaT = dateAdd("d",diasP,FECHA_INST)
	
	Response.Write "<dataset seriesName='PROYECCION " &  fechaT & " Tasa " & round(tasa,3) & "mm/dia   Media " & round(media,3) & " Min " & min & "'  showValues='0' anchorRadius='2' color='FFCC00'>"  & VbCrLf
	...
	While i<=j 'GRAFICAR
			...
			if diasOP > 0 then
				...
				desgastePr = CUBIERTA - (desgaste / 1) * diasP
			end if
		
		 	Response.Write "<set label='" & i & "' value='" & round(desgastePr,0) & "' />" & VbCrLf
	...
	'if (fechaT <> "") then
		...
		mex = arrdet(1)
		dex = arrdet(0) 
		arrdet2 = split(FECHA_INST, "/")
		yex2 = arrdet2(2)
		mex2 = arrdet2(1)
		dex2 = arrdet2(0) 
		
		
		query = "UPDATE MEDICIONES SET FECHA_CAMBIO = '" & dex & "/" & mex & "/" & yex & "' "
		query = query & " WHERE  (FOLIO = " & FOLIO & ")"
	...
	IF LEN(TRIM(FOLIO_R))>0 THEN
		...
		i=1
		Response.Write "<dataset seriesName='RETORNO'  showValues='0' color='000000' anchorRadius='1'>" & VbCrLf
		While Not ORsRetorno.eof 
		 	Response.Write "<set label='" & i & "' value='" & ORsRetorno.Fields.Item(1).Value & "' />" & VbCrLf
			ORsRetorno.MoveNext
			i=i+1
		Wend
		Response.Write "</dataset>" & VbCrLf
	...
	END IF ' RETORNO
   	...
   	Response.Write "<trendLines>" & VbCrLf
    Response.Write "<line startValue='0' color='FF0000' displayvalue='CARCASA' valueOnRight='1'/> " & VbCrLf
	Response.Write "</trendLines>" & VbCrLf
	
	Response.Write "<trendLines>" & VbCrLf
    Response.Write "<line startValue='" & ESPESOR_SEG & "' color='FFFF00' displayvalue='SEGURIDAD' valueOnRight='1'/> " & VbCrLf
	Response.Write "</trendLines>" & VbCrLf
	
	Response.Write "<trendLines>" & VbCrLf
    Response.Write "<line startValue='" & CUBIERTA & "' color='000099' displayvalue='CUBIERTA' thickness='1' valueOnRight='1' showOnTop='1'/> " & VbCrLf
	Response.Write "</trendLines>" & VbCrLf
	
	Response.Write "<trendLines>" & VbCrLf
    Response.Write "<line startValue='" & FONDO & "' color='000099' displayvalue='RETORNO' thickness='1' valueOnRight='1' showOnTop='1'/> " & VbCrLf
	Response.Write "</trendLines>" & VbCrLf
