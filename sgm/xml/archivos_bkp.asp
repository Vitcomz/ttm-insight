...
function ListArchivos()
	...
	query = "SELECT dbo.ARCHIVOS.ID, dbo.ARCHIVOS.NOMBRE, dbo.ARCHIVOS.EXT, RTRIM(CONVERT(CHAR, dbo.ARCHIVOS.FECHA, 103)) AS FECH, dbo.ARCHIVOS.RUTA, dbo.ARCHIVOS.FAMILIA, dbo.ARCHIVOS.IDENTIFICADOR, dbo.ARCHIVOS.SIZE, dbo.ARCHIVOS.EXT AS EXT2 "
	query = query & " FROM dbo.ARCHIVOS INNER JOIN dbo.BITACORAS ON dbo.ARCHIVOS.IDENTIFICADOR = dbo.BITACORAS.ID "
	query = query & " WHERE FAMILIA = 'BITACORAS'" 
	query = query & " and BITACORAS.ubicacion in (" & strNvl(xFaena) & ")"
...
function NewArchivo
	...
	Set ORs =oDB.EjecutaSql(query)
	If not ORs.eof Then
		IDENTIFICADOR = ORs.Fields.Item(0).Value
	End If

	query = "SELECT ID AS NUM FROM ARCHIVOS "
	query = query & "WHERE IDENTIFICADOR='" & IDENTIFICADOR & "' and FAMILIA = 'BITACORAS' and NOMBRE = '" & NOMBRE & "'"
	Set ORs =oDB.EjecutaSql(query)
	If not ORs.eof Then
		N = 1
	Else
	...
	if N = 0 then
		...
		query = query & strNvl("image/jpeg") & ","
		query = query & strNvl(SIZX) & ","
...
'window.opener.GetValueFromChild('" & N & "');  
if(WIN = "OK") then
	 Response.Write ("<html><body><script>setTimeout(function(){window.close(); }, 700);</script></body></html>")
	' window.opener.document.getElementById('hddvalueid').value = '" & session("idx") & "' ;
end if
