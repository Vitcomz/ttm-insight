...
function LayoutFaena()
	...
	for loopctr = 0 to ubound(xfan)
		xxfaena = xfan(loopctr)
		query = query & " (SELECT DBO.EquipoPrincipalFaena(B.EQUIPO," & strNvl(xxfaena)  & ") AS EQUIPO, MAX(B.CRITICIDAD) *  T.CRITICIDAD/100 AS CRITICIDAD "
		query = query & " FROM BITACORAS AS B, EQUIPOS AS E, TIPO_EQUIPOS AS T WHERE E.ESTADO='ACT' AND B.UBICACION =  " & strNvl(xxfaena) 
		query = query & " AND B.EQUIPO=E.CODIGO AND E.TIPO_EQUIPO = T.CODIGO AND B.COLOR IS NOT NULL "
		query = query & " AND IND_SOLUCION='false' GROUP BY DBO.EquipoPrincipalFaena(B.EQUIPO," & strNvl(xxfaena)  & "),T.EQUIPOCRITICIDAD, T.CRITICIDAD) "
		if (loopctr < ubound(xfan)) then
			query = query & " UNION "
		end if
	next
	query = query & " ) AS T "
	query = query & " ON T.EQUIPO = E.CODIGO WHERE E.UBICACION in (" & strNvl(xFaena) & ") " 
	query = query & " AND E.COD_EQUIPO_PADRE IS NULL  "
	query = query & " AND E.ESTADO = 'ACT' "
	query = query & " GROUP BY DBO.EquipoPrincipal(E.CODIGO), E.SECUENCIA, E.DESCRIPCION, E.LAYOUT, E.X, E.Y, E.SCALEX, E.SCALEY, E.UBICACION "
	query = query & " ORDER BY E.UBICACION, E.SECUENCIA, DBO.EquipoPrincipal(E.CODIGO)"
...
function ListBitacora()
	...
	query = query & " B.ULT_INSPECCION, B.OT, B.ULT_MOD_OBS, B.FECHA_PROGRAMADA, B.CONDICION, B.STATUS "
...
'	query = query & " DATEADD(d,1," & strNvl(FECHA_FIN) & ")) AND "
	query = query & " B.UBICACION in (" & strNvl(xFaena) & ")"
...
'				query = query & " AND DBO.SecEquipoPrincipal(B.EQUIPO) IN (" & SECCION & ")"
	...
	While Not ORs.EOF 
	  ...
	  Response.Write "<ROW>" 
		  ...
		  Response.Write "<EQUIPO_PRIN>" & trim(ORs.Fields.Item(14).Value) & "</EQUIPO_PRIN>"
		  ...
		  Response.Write "<EJECUTOR>" & ORs.Fields.Item(20).Value & "</EJECUTOR>"
  		  ...
  		  Response.Write "<CONDICION>" & ORs.Fields.Item(33).Value & "</CONDICION>"
  		  Response.Write "<FECHA_PROGRAMADA>" & ORs.Fields.Item(32).Value & "</FECHA_PROGRAMADA>" 
		  Response.Write "<STATUS>" & ORs.Fields.Item(34).Value & "</STATUS>"
		 ...
		 Set oRsFiles = getFiles(ORs.Fields.Item(0).Value)
		  If Not oRsFiles.EOF Then
		  		If EXCEL = "YES" Then
					Response.Write "<FILE>../Files/" & trim(ORsFiles.Fields.Item(0).Value) & "</FILE>"
				Else
		  			Response.Write "<FILE>Files/" & trim(ORsFiles.Fields.Item(0).Value) & "</FILE>"
				End If
		  Else
...
function getLayout()
	...
	query = query & ", Orden = "
	query = query & " CASE WHEN MAX(B.CRITICIDAD) > 66.7 THEN '1' WHEN MAX(B.CRITICIDAD) > 33.3 THEN '2' ELSE '3' END , ISNULL(E.SECUENCIA,1), B.UBICACION"
	...
	query = query & " and E.LAYOUT is not null AND  B.UBICACION in (" & strNvl(xFaena) & ")"
...
'	End if	
	...
	query = query & " GROUP BY DBO.EquipoPrincipalFaena(B.EQUIPO, B.UBICACION),E.SECUENCIA, E.GCODE, E.LAYOUT, X, Y, B.UBICACION "
	query = query & " ORDER BY B.UBICACION, DBO.EquipoPrincipalFaena(B.EQUIPO, B.UBICACION),E.SECUENCIA, ORDEN "
	...
	While Not ORs.EOF 
		'if ORs.Fields.Item(0).Value <> equipoValor THEN'
			...
			Response.Write "<N>" & N & "</N>"
			Response.Write "<EQUIPO>" & ORs.Fields.Item(0).Value & "</EQUIPO>"
			Response.Write "<EQUIPO_PRIN>" & trim(ORs.Fields.Item(1).Value) & "</EQUIPO_PRIN>"
			...
			Response.Write "<SECUENCIA>" & ORs.Fields.Item(9).Value & "</SECUENCIA>"
			Response.Write "<FAENA>" & ORs.Fields.Item(10).Value & "</FAENA>"
			Response.Write "</ROW>"
		'End if'
		'equipoValor = ORs.Fields.Item(0).Value'
	  ORs.MoveNext 
...
function getPlanData()
	...
	query = "SELECT B.REPORT, B.OT, B.FECHA_PROGRAMADA, RECOMENDACION, ACCION, CONDICION, STATUS, B.ID, B.UBICACION"
	query = query & " FROM BITACORAS AS B "
	'query = query & " WHERE B.ID = " & strNvl(ID)
	query = query & " WHERE  B.UBICACION in (" & strNvl(xFaena) & ") order by B.UBICACION"
	...
	While Not ORs.EOF 
	  Response.Write "<ROWS>" 
		  ...
		  Response.Write "<FAENA>" & ORs.Fields.Item(8).Value & "</FAENA>"
		  Response.Write "<OP></OP>"
	  Response.Write "</ROWS>"
	  ORs.MoveNext
	Wend
