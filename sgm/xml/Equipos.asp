...
Set oDB = New DBConn
	...
	ESPESOR_SEG=REQUEST("ESPESOR_SEG")
...
function ListEquiposPadres()
	...
	query = "SELECT E.CODIGO, E.DESCRIPCION, E.LAYOUT, COUNT(MED.FOLIO) AS LECTURAS "
	query = query & " FROM EQUIPOS AS E "
	query = query & " LEFT OUTER JOIN MEDICIONES AS MED ON E.UBICACION = MED.FAENA AND E.CODIGO = MED.EQUIPO "
	...
	While Not ORs.EOF 
	  Response.Write "<ROW>"  '& query
		  ...
		  Response.Write "<LECTURAS>" & ORs.Fields.Item(3).Value & "</LECTURAS>"
  	   Response.Write "</ROW>"
	  ORs.MoveNext
	Wend
	Response.Write "</REGISTROS>"
...
function GetTipo()
	...
	query = "SELECT E.TIPO_EQUIPO, TE.DESCRIPCION, E.COD_EQUIPO_PADRE, E.DESCRIPCION "
	...
	If Not ORs.EOF Then
	  Response.Write "<ROWS>" 
		  ...
		  Response.Write "<EQDESCRIPCION>" & ORs.Fields.Item(3).Value & "</EQDESCRIPCION>"
	  Response.Write "</ROWS>"
	End IF
	Response.Write "</REGISTROS>"
	Set ORs = Nothing
...
Session.LCID = 2057
	...
	query = "SET DATEFORMAT DMY SELECT E.CODIGO, E.ESTADO, E.DESCRIPCION, E.HOROMETRO, E.HOROMETRO_MAN, "
	...
	query = query & " E.ESPESOR, E.TON_SEMANAL, E.ESPESOR_R, E.ESPESOR_SEG "
	query = query & "FROM "
	query = query & "  (SELECT E.*, Equipos.DESCRIPCION AS DESC_PADRE "
	query = query & "  FROM EQUIPOS AS E LEFT JOIN Equipos ON E.COD_EQUIPO_PADRE = Equipos.CODIGO "
	...
	If Not ORs.EOF Then
	  Response.Write "<ROW>" 
		  ...
		  Response.Write "<ESPESOR_SEG>" & ORs.Fields.Item(40).Value & "</ESPESOR_SEG>"
	  Response.Write "</ROW>"
	End If
	Response.Write "</REGISTROS>"
	Set ORs = Nothing
...
function UpdateEquipo()
	...
	query = query & "ESPESOR_SEG=" & strNvl(ESPESOR_SEG) & ", "
